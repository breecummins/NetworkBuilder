...
def makeHistogram(fname="stats.txt",histtitle='8D LEM network, 154 perturbations',scaling_factor='0.00',startingvals=[float('nan')]*3):
    ...
    plotHist(FCoverParams,'% stable FCs',histtitle,float(startingvals[0]))
    plotHist(MatchesOverParams,'% matches over total parameters',histtitle+', scaling factor '+scaling_factor,float(startingvals[1]))
...
def makeScatterPlot(fname1="stats_0-00.txt",fname2="stats_0-05.txt",startingvals=[(float('nan'),float('nan'))]*3,title='8D LEM network',scaling_factor1='0.00',scaling_factor2='0.05'):
    ...
    def plotScatter(data1,data2,fulltitle,xlabel,ylabel,sv):
