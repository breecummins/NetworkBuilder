...
mkdir -p $INPUTDIR/networks $INPUTDIR/POs $OUTPUTDIR/
...
for NETWORK in $( ls $INPUTDIR/networks/*); do
	...
	qsub shuffledgenes_helperscript.sh $PATTERNMATCH $NETWORK $PATTERNFILE $OUTPUTDIR $NUM $STABLEFCS $MULTISTABLE $NODES $RESULTSFILE 
done
